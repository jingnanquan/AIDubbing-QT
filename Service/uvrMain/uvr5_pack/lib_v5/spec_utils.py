...
def wave_to_spectrogram_mt(wave, hop_length, n_fft, mid_side=False, mid_side_b2=False, reverse=False):
    ...
    def run_thread(result_container: list, **kwargs):
        # global spec_left
        # spec_left = librosa.stft(**kwargs)
        result_container.append(librosa.stft(**kwargs))

    spec_container = []
    thread = threading.Thread(target=run_thread, args=(spec_container,), kwargs={'y': wave_left, 'n_fft': n_fft, 'hop_length': hop_length})
    thread.start()
    spec_right = librosa.stft(wave_right, n_fft= n_fft, hop_length=hop_length)
    thread.join()   

    spec_left = spec_container[0]
    spec = np.asfortranarray([spec_left, spec_right])
