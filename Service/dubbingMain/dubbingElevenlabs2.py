...
class dubbingElevenLabs2(dubbingInterface):
    """
    ...
    def dubbing_high_quality(self, vocal_path: str, back_path: str, source_subs: list, target_subs: list,
                             role_match_list: list, video_path: str, voice_param: dict, on_progress=None) -> dict:
        '''
        ...
        try:
            ...
            try:
                j = 1
                sentence = ""
                for subtitle in source_subs:   # 根据目标配音字幕，生成原字幕的text
                    sentence += subtitle["text"]
                    if subtitle["end"] == dubbing_subs[str(j)]["end"]:  # 这里是以dict访问的
                        source_dubbing_subs.append(sentence)
                        sentence = ""
                        j += 1
            except Exception as e:
                print(f"Error parsing subtitle: {e}")
                source_dubbing_subs = list(dubbing_subs.values())
            print(source_dubbing_subs)


            if on_progress:
                on_progress(30, "角色干音分片中...")
                    # 使用eleven获取更纯净的人声
            pure_vocal_path = os.path.join(AUDIO_SEPARATION_FOLDER, "pure_vocal_{}.wav".format(timestamp))
            if self.connect.video_voice_isolate(video_path, pure_vocal_path): vocal_path = pure_vocal_path  # 替换掉人声路径
            role_subtitles, vocal_audio, _, role_audio_path = self.parse_roles_numpy_separate(source_subs,
                                                                                              role_match_list,
            ...
            for i, subtitle in enumerate(dubbing_subs.values()):
                ...
                audio = self.connect.elevenlabs.text_to_speech.convert(
                    text=subtitle["text"],
                    voice_id=voice_id,
                ...
                if res_frames - source_frames > 10000:  # 超出太多，就应该加速，大概冗余0.22s
                    print("时长超出太多，应该加速")
                    speed = res_frames / (source_frames + 10000)
                    # 使用librosa加速音频
            ...
            result_dir = os.path.join(RESULT_OUTPUT_FOLDER,
                                      "{}-视频一键配音结果-{}".format(os.path.basename(video_path).split('.')[0],
                                                                      timestamp))  # 创建一个文件夹
            os.makedirs(result_dir, exist_ok=True)

            output_audio_file = os.path.join(result_dir, "音频-配音-{}.mp3".format(timestamp))
            target_voice_audio_path = os.path.join(result_dir, "音频-配音纯人声-{}.mp3".format(timestamp))
    ...
    def dubbing_mixing_cocktails(self, vocal_path: str, back_path: str, source_subs: list, target_subs: list,
                             role_match_list: list, video_path: str, voice_param: dict, output_path: str, on_progress=None) -> dict:
        '''
        我在这里加上了当前配音的前两句，以提高生成的稳定性, 但是其实是基于音素切分的，它实际上是生成了更长的音频，然后裁剪的
        '''
        back_audio, samplerate = sf.read(back_path)
        ...
        try:
            ...
            for i, subtitle in enumerate(dubbing_subs.values()):
                ...
                res_audio = np.array(dub_audio.get_array_of_samples())
                res_audio = res_audio.astype(np.float64) / 32768.0
                temp_audio = np.vstack([res_audio, res_audio]).T
                if all_audio is None:
                    all_audio = np.copy(temp_audio)
                else:
                    empty_array = np.zeros((88200, 2), dtype=temp_audio.dtype)  # 2s缓冲
                    all_audio = np.concatenate([all_audio, empty_array, temp_audio])
            ...
            output_audio_file = os.path.join(result_dir, "音频-人声+背景-{}.mp3".format(timestamp))
            target_voice_audio_path = os.path.join(result_dir, "音频-纯人声-{}.mp3".format(timestamp))
            target_initial_voice_audio_path = os.path.join(result_dir, "音频-纯人声无加速无对齐-{}.mp3".format(timestamp))
            output_video_file = os.path.join(result_dir, "视频-配音-{}.mp4".format(timestamp))
            target_subtitles_path = os.path.join(result_dir, "合并后的配音字幕.txt")
            modified_subtitles_path = os.path.join(result_dir, "字幕-cps优化后字幕.txt")
    ...
    def dubbing_without_clone3(self, target_subs: list, role_match_list: list, video_path: str, voice_param: dict, output_path: str=RESULT_OUTPUT_FOLDER, cps: str="", on_progress=None, delete=False) -> dict:
        '''
        我在这里加上了当前配音的前两句，以提高生成的稳定性, 但是其实是基于音素切分的，它实际上是生成了更长的音频，然后裁剪的
        '''
        ...
        try:
            ...
            # 使用eleven获取更纯净的人声
            # pure_vocal_path = os.path.join(result_dir, "pure_vocal_{}.wav".format(timestamp))
            # self.connect.video_voice_isolate(video_path, pure_vocal_path)
            # vocal_path = pure_vocal_path  # 替换掉人声路径
            ...
            role_subtitles, vocal_audio, _, role_audio_path = self.parse_roles_numpy_separate(target_subs, role_match_list, vocal_path, voice_param, output_path= result_dir)
            if on_progress:
                print("角色声音克隆中...")
                on_progress(35, "角色声音克隆中...")
            voice_ids = self.batch_clone_text(role_subtitles, role_audio_path, timestamp, voice_param)
            ...
            for i, subtitle in enumerate(dubbing_subs.values()):
                ...
                while circle and retry > 0:
                    ...
                    # print(audio.data.normalized_alignment)
    ...
    def adjust_speed(self, input_audio: np.ndarray, speed:float, characters:list, time_seconds:list, target_frames:int, sr=44100, up_tolerance=None):
        ...
        if speed > 1.15:
            ...
            # target_frames_with_tolerance = int(target_frames * 0.85)
            # current_frames = res_audio.shape[0]
            # if target_frames_with_tolerance > current_frames:
            #     frames_to_add = target_frames_with_tolerance - current_frames
            #     space_indices = [i for i, char in enumerate(characters) if char == " " and i < len(time_seconds) - 1]
            #     if space_indices:
            #         frames_per_space = frames_to_add // len(space_indices)
            #         if frames_per_space>0:
            #             # 计算每个空格需要增加的帧数
            #             # 在每个空格位置插入空白音频
            #             new_audio_parts = []
            #             last_end_frame = 0
            ...
            #                 start_frame = int(start_time * sr)
            #                 end_frame = int(end_time * sr)
            #                 new_audio_parts.append(res_audio[last_end_frame: start_frame])
            #
            #                 space_audio1 = res_audio[start_frame: (start_frame+end_frame)//2]
            #                 new_audio_parts.append(space_audio1)
            #
            #                 silence_audio = np.zeros((frames_per_space, 2))  # 双声道空白音频
            #                 new_audio_parts.append(silence_audio)
            #
            #                 space_audio2 = res_audio[(start_frame+end_frame)//2: end_frame]
            #                 new_audio_parts.append(space_audio2)
            #                 last_end_frame = end_frame
            #             # 添加最后一段音频
            #             new_audio_parts.append(res_audio[last_end_frame:])
            #             # 合并所有部分
            #             res_audio = np.vstack(new_audio_parts)
            use_speed = max((res_audio.shape[0]+7000)/target_frames, 0.96)
            print("!减速后--speed", use_speed)
        else:
    ...
    @classmethod
    def getInstance(cls) -> "dubbingElevenLabs2":
        if not cls._instance:
            cls._instance = dubbingElevenLabs2()
...
if __name__ == '__main__':
    eleven = dubbingElevenLabs2.getInstance()
