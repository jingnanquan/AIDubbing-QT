...
class SubtitleInterface(Ui_SubtitleEdit, FramelessWindow):
    ...
    def set_video_voice_changer_params(self):
        ...
        self.params_window = VoiceChangerParamsWindow(self.subtitlePaths, role_match_list,
                                                      self.DubbingSelector.currentIndex() + 1, self.video_file,
                                                      video_duration)
    ...
    def set_cosy_dubbing_params(self):
        ...
        for item in items:
            role_match_list.append(item.roles.currentText())
        # 创建参数窗口, 需要self持有这个窗体，否则会被清除
        self.params_window = CosyDubbingParamsWindow(self.subtitlePaths, role_match_list,
                                                     self.DubbingSelector.currentIndex() + 1, self.video_file,
                                                     video_duration)
        self.params_window.setWindowModality(Qt.ApplicationModal)
        self.params_window.show()  # 显示
    ...
    def set_directed_dubbing_params(self):
        ...
        for item in items:
            role_match_list.append(item.roles.currentText())
        # 创建参数窗口, 需要self持有这个窗体，否则会被清除
        self.params_window = DirectedDubbingParamsWindow(self.subtitlePaths, role_match_list,
                                                         self.DubbingSelector.currentIndex() + 1, self.video_file,
                                                         video_duration)
        self.params_window.setWindowModality(Qt.ApplicationModal)
        self.params_window.show()  # 显示
    ...
    def _setup_unfinished_ui(self):
        ...
        self.SubScroll.setWidget(self.SubListContainer)
        # self.SubScroll.setStyleSheet(
        #     """ #SubScroll{ border: None; background: transparent; } #SubtitleCardContainer{ background: transparent; } """)
