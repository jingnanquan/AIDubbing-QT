...
from qfluentwidgets import (
    ...
    PrimaryPushButton,
    Theme,
    setTheme, Slider, HyperlinkButton,
)
...
class DubbingEditorInterface(FluentWindow):
    def __init__(self) -> None:
        ...
        self.titleBar.hBoxLayout.insertWidget(2, self._title_btn, 0, Qt.AlignCenter)
        self.titleBar.hBoxLayout.addStretch()
        self.titleBar.hBoxLayout.setSpacing(10)
        self.titleBar.setContentsMargins(5, 0, 5, 0)

        
        # main_content_layout.addLayout(button_layout, 0)
        main_content_layout.addWidget(self._split, 1)
        main_content_layout.addWidget(slider_container, 0)
        main_content_layout.addWidget(self._edit_panel, 0)
    ...
    def _on_import_project(self, folder: str="") -> None:
        ...
        if not result.ok or not result.paths:
            box = MessageBox("导入失败", "项目导入失败", self)
            box.exec()
            return
        ...
        subs, roles_raw = parse_subtitle_uncertain(self._paths.subtitle)
        if not subs:
            box = MessageBox("导入失败", "字幕解析失败或文件为空", self)
            box.exec()
            return
    ...
    def _on_submit_requested(self, idx: int, text: str, speed: float) -> None:
        if not self._paths or idx < 0 or idx >= len(self._subtitles):
        ...
        start_ms = timecode_to_ms(self._subtitles[idx]["start"])
        end_ms = timecode_to_ms(self._subtitles[idx]["end"])
    ...
    def _on_save_video(self) -> None:
        ...
        # 检查必要的文件是否存在
        if not self._paths.dubbed_video or not self._paths.original_video:
            InfoBar.error(
                title="保存失败",
                content="缺少必要的视频文件",
                orient=Qt.Horizontal,
        ...
        # 生成输出文件名
        base_dir = self._paths.workspace_dir
        dubbed_output = os.path.join(base_dir, f"{self._paths.project_name}_配音输出.mp4")
        original_output = os.path.join(base_dir, f"{self._paths.project_name}_原视频输出.mp4")
        
        try:
            # 保存配音视频：bgm + dub_vocal_audio
            if self._paths.background and self._paths.dub_vocal_audio:
                self._merge_audio_to_video(
                    self._paths.dubbed_video,
                ...
                InfoBar.success(
                    title="配音视频已保存",
                    content=f"已保存到: {os.path.basename(dubbed_output)}",
                    orient=Qt.Horizontal,
                    isClosable=True,
                    position=InfoBarPosition.TOP_RIGHT,
                    duration=3000,
            ...
            # 保存原视频：bgm + vocal
            if self._paths.background and self._paths.vocal:
                self._merge_audio_to_video(
                    self._paths.original_video,
                    self._paths.background,
                    self._paths.vocal,
                    original_output
                )
                InfoBar.success(
                    title="原视频已保存",
                    content=f"已保存到: {os.path.basename(original_output)}",
                    orient=Qt.Horizontal,
                    isClosable=True,
                    position=InfoBarPosition.TOP_RIGHT,
                    duration=3000,
