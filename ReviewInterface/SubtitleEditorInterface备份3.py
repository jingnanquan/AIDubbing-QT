...
from PyQt5.QtCore import Qt, QPropertyAnimation, QPoint, QTimer, QProcess, QUrl, QElapsedTimer
...
class SubtitleEditorInterface(Ui_SubtitleEdit, FramelessWindow):
    ...
    def __init__(self, parent=None):
        ...
        # self.BottomPositionSlider = qfluentwidgets.Slider()
    ...
    def _init_video_player(self):
        ...
        try:
            ...
            # 简单探测 ffplay 是否可用：尝试启动一次版本命令会更准，但这里先保持轻量
        except Exception:
            self._player_backend = None
    ...
    def show_subtitle_list(self, item):
        ...
        self.mark_clean()
        items = self.findChildren(SubtitleListItemEdit)
    ...
    def on_subtitle_insert_below(self, row: int):
        ...
        base = self.subtitles[row] if (0 <= row < len(self.subtitles)) else (self.subtitles[-1] if self.subtitles else {})
        new_sub = {
            "index": insert_at + 1,
