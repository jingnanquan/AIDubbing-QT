        ...
        try:
            ...
            for idx, (video_path, srt_path) in enumerate(self.pairs):
                try:
                    ...
                    vad_separate_audios = []
                    origin_subtitles_0, _ = parse_subtitle_uncertain(srt_path)
                    origin_subtitles = LLMAPI.getInstance().merge_subtitle_with_audio(origin_subtitles_0, video_path)
                    print(origin_subtitles)
                    ...
                    if origin_subtitles:
                        ...
                        if len(embs) > 0 and len(labels) > 0:
                            try:
                                self.progress.emit(completed, f"正在创建高级交互式可视化...")
                                ...
                                viz_process = launch_visualization_safely(
                                    embs, labels, vad_separate_audios, 16000, subtitle_info
                                )
                                
                                if viz_process:
                                    print(f"高级可视化进程已启动，PID: {viz_process.pid}")
                                else:
                                    print("可视化进程启动失败")
